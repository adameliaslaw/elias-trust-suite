...
> 252-check smoke suite. PR 3 MERGED (#38, `cdcd631`): expenses route group extracted (same pattern). PR 4 open:
> extract the customers route group (4 handlers).**
...
| 6 — Books role + `packages/rules` | #25 | 🟨 **In progress** — PR 1 MERGED (#36): `@elias/rules` + payroll retrofit + tax fixes (6/8). PR 2 MERGED (#37): `server.js` split begun (sales-tax + reports group). PR 3 MERGED (#38): expenses group. PR 4 open: customers group extracted |
